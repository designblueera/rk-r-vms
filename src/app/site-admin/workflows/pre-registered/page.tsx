...
interface PreRegisteredWorkflow {
  id: number;
  name: string;
  visitorType: string;
  status: 'active' | 'draft';
...
// Workflows from Global Overview (pre-registered)
const initialWorkflows: PreRegisteredWorkflow[] = [
  {
    id: 1,
...
export default function SiteAPreRegisteredWorkflowsPage() {
  const router = useRouter();
  const { siteName } = useRole();
  const [workflows, setWorkflows] = useState<PreRegisteredWorkflow[]>(initialWorkflows);
  const [showTypeModal, setShowTypeModal] = useState(false);
  const [openMenuId, setOpenMenuId] = useState<number | null>(null);

  const handleSelectVisitorType = (typeName: string) => {
    setShowTypeModal(false);
    router.push(`/site-admin/workflows/pre-registered/builder?visitorType=${encodeURIComponent(typeName)}`);
  };

  const handleEditWorkflow = (w: PreRegisteredWorkflow) => {
    router.push(`/site-admin/workflows/pre-registered/builder?visitorType=${encodeURIComponent(w.visitorType)}&workflowId=${w.id}&workflowName=${encodeURIComponent(w.name)}`);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            return (
              ...
              >
                ...
                <div className="px-5 py-3 border-t border-border flex items-center justify-between bg-surface/50 rounded-b-xl">
                  ...
                  <button
                    onClick={() => router.push(`/site-admin/workflows/pre-registered/builder?visitorType=${encodeURIComponent(w.visitorType)}&workflowId=${w.id}&mode=view`)}
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-[11px] text-text-muted mt-0.5">Create a new pre-registered workflow</p>
            </div>
          </button>
        </div>
      </div>
